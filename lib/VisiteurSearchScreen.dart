...
import 'VisiteurAppo.dart';
import 'package:cached_network_image/cached_network_image.dart';

class VisiteurSearchScreen extends StatefulWidget {
  


   @override
  _VisiteurSearchScreenState createState() => _VisiteurSearchScreenState();
}

class _VisiteurSearchScreenState extends State<VisiteurSearchScreen> { 
   String _searchQuery = '';
  ...
  Future<void> _requestLocationPermission() async {
    ...
    if (status != PermissionStatus.granted) {
      // Gérer le cas où l'utilisateur n'a pas accordé la permission
      showDialog(
        context: context,
    ...
    try {
      ...
      showDialog(
        ...
        builder: (BuildContext context) => AlertDialog(
          title: Text("Error"),
          content:
              Text("Failed to get current location. Please try again later."),
          actions: [
            TextButton(
              onPressed: () {
                Navigator.pop(context);
      ...
      child: InkWell(
        onTap: () {
          if (route == '/appointment') {
            Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => VisiteurAppo()),
            );
          } else if (route == '/nurse') {
            Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => NurseAcceuilPage()),
          );
  ...
  Future<List<DropdownMenuItem<String>>> _getSpecialitesFromFirestore(
    ...
    try {
      ...
      if (querySnapshot.size == 0) {
        return [
          DropdownMenuItem<String>(
              value: 'No Data', child: Text('No data found'))
        ];
      }
